function deleteCompany(companyID) {
    ...
    xhttp.open("DELETE", "/delete-company-ajax", true);
...
function deleteRow(companyID) {
    ...
    for (let i = 0, row; row = table.rows[i]; i++) {
        ...
        if (table.rows[i].getAttribute("data-value") == companyID) {
            table.deleteRow(i);
            deleteDropDownMenu(companyID);
            break;
        }
    }
}

function deleteDropDownMenu(companyID) {
    let selectMenu = document.getElementById("mySelectCompany");
    for (let i = 0; i < selectMenu.length; i++) {
        if (Number(selectMenu.options[i].value) === Number(companyID)) {
            selectMenu[i].remove();
            break;
        }
